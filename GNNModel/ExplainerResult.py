...
def explain():
    ...
    node_importance = (features.cpu() * feat_mask.cpu()).sum(dim=1).detach().numpy()
    most_important_node = np.argmax(node_importance)
    most_important_feature = single_graph.ndata['instruction_features'][most_important_node]
    
    print(f"Most important node ID: {most_important_node}")
    print(f"Node importance value: {node_importance[most_important_node]}")
    print(f"Node features: {most_important_feature}")
